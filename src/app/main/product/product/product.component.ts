...
export class ProductComponent extends BaseComponent implements OnInit {
  ...
  onDelete(row) {
    this._api.post('/api/item/delete-item', { masp: row.masp }).takeUntil(this.unsubscribe).subscribe(res => {
  ...
  public openUpdateModal(row) {
    ...
    setTimeout(() => {
      $('#createItemModal').modal('toggle');
      this._api.get('/api/item/get-by-id/' + row.masp).takeUntil(this.unsubscribe).subscribe((res: any) => {
        this.item = res;
        this.formdata = this.fb.group({
          'item_image': [this.item.item_image, Validators.required],
          'tensp': [this.item.tensp],
          'gia': [this.item.gia, Validators.required],
          'motangan': [this.item.motangan],
        }, {
        });
        this.doneSetupForm = true;
      });
